...
class InventoryApp:
    ...
    def setup_products_tab(self):
        ...
        self.tree_prod.tag_configure('odd', background='#f7f9ff')
        self.tree_prod.tag_configure('even', background='#ffffff')
    ...
    def setup_sales_tab(self):
        ...
        # Alternate row coloring for better readability
        self.tree_sales.tag_configure('odd', background='#f7f9ff')
        self.tree_sales.tag_configure('even', background='#ffffff')
    ...
    def refresh_sales(self):
        ...
        for idx, row in enumerate(cursor.fetchall()):
            formatted_row = (row[0], row[1], row[2], f"₱{row[3]:.2f}", row[4])
            tag = 'odd' if idx % 2 == 0 else 'even'
            self.tree_sales.insert("", tk.END, values=formatted_row, tags=(tag,))

if __name__ == "__main__":
    root = tk.Tk()
    root.configure(bg="#e6eefc")

    style = ttk.Style()
    style.theme_use('clam')
    
    # Apply a consistent color theme
    style.configure('TNotebook', background='#e6eefc')
    style.configure('TNotebook.Tab', background='#dce7f8', foreground='#1a1a1a')
    style.configure('TFrame', background='#e6eefc')
    style.configure('TLabel', background='#e6eefc', foreground='#1a1a1a')
    style.configure('TLabelframe', background='#dce7f8', foreground='#1a1a1a')
    style.configure('TLabelframe.Label', background='#dce7f8', foreground='#1a1a1a')
    style.configure('TEntry', fieldbackground='#ffffff', background='#ffffff', foreground='#1a1a1a')
    style.configure('TButton', background='#4a90e2', foreground='#ffffff', borderwidth=1)
    style.map('TButton', background=[('active', '#3a78c2'), ('pressed', '#2f63a3')])
